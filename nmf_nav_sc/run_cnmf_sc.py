...
def run_cnmf_sc():
    ...
    st.subheader("Upload Single-Cell Dataset")
    upload_counts()
    ...
    st.session_state["gene_column_sc"] = st.text_input("Type in name of the column that stores gene names")

    st.session_state["batch_sc"] = st.checkbox(
        "Check the box if you would like to run batch correction",
        value=False
    )
    ...
    if st.session_state["batch_sc"]:
        ...
        # st.session_state["batch_column_sc"] = st.selectbox(
    ...
    st.session_state["gene_symbols_sc"] = st.checkbox(
        "Check the box if your transcript names are gene symbols (instead of Ensembl IDs).",
        value=False
    )
    ...
    if st.button("Run CNMF") and not st.session_state["cnmf_running_sc"]:
        ...
        post_data = {
           ...
           # "max_iter": int(max_iter),
            ...
            "batch_correct": st.session_state["batch_covars_sc"],
    ...
    if st.session_state.get("display_scores_sc", False):
        ...
        csv_module_usages = convert_for_download(st.session_state["cnmf_module_usages_sc"])
        st.download_button(
            label="Download CSV",
            data=csv_module_usages,
            file_name="data.csv",
            mime="text/csv",
    ...
    if st.session_state.get("display_loadings_sc", False):
        st.subheader("Gene Loadings Matrix (H) Preview")
        st.dataframe(st.session_state["cnmf_gene_loadings_sc"].iloc[:20, :20])

        csv_gene_loadings = convert_for_download(st.session_state["cnmf_gene_loadings_sc"])
        st.download_button(
            label="Download CSV",
            data=csv_gene_loadings,
            file_name="data.csv",
            mime="text/csv",
            icon=":material/download:",
        )

...
def run_gene_loadings_sc():
    apply_custom_theme()

    if "results_summary_sc" not in st.session_state:
        st.session_state["results_summary_sc"] = None
    df = None

    if "cnmf_gene_loadings_sc" not in st.session_state:
        st.subheader("Please run one of the NMF algorithms to obtain gene loadings data")
    else:
        if "cnmf_gene_loadings_sc" in st.session_state and st.session_state["cnmf_gene_loadings_sc"] is not None:
            df = st.session_state["cnmf_gene_loadings_sc"]
        else:
            st.warning("Please, run consensus NMF to obtain gene loadings information before proceeding with this step")
            ...
            if st.button("Summarise gene functions"):
                with st.spinner("Summarising gene functions..."):
                    response = requests.post(
                        st.session_state["API_URL_sc"] + "/process_gene_loadings/",
                        files=files,
                        data=data
                    )

                    if response.status_code == 200:
                        st.session_state["results_summary_sc"] = response.json()
                        st.success("Results received!")
                    else:
                        st.error(f"Backend error {response.status_code}: {response.text}")

        if st.session_state.get("results_summary_sc"):
            results = st.session_state["results_summary_sc"]

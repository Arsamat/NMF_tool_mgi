...
class cNMF():
    ...
    #custom function to save artifacts
    def prepare_from_preprocessed(
        self,
        X,                     # cells x genes (pd.DataFrame, np.ndarray, or AnnData)
        components,            # list of K values
        n_iter=100,
        seed=None,
        beta_loss="frobenius",
        alpha_usage=0.0,
        ...
        init="random",
        max_NMF_iter=1000,
        tpm=None,              # optional: cells x genes (same types as X). If None, uses X.
        densify=False,         # if X is DataFrame/ndarray, store as sparse unless densify=True
    ):
        """
        Like prepare(), but assumes X is already preprocessed/normalized.
        Writes the artifacts needed for downstream steps without recomputing TPM,
        HVGs, or scaling to unit variance.
        """

        self._initialize_dirs()

        # --- Convert X to AnnData (cells x genes) ---
        if "AnnData" in str(type(X)):
            norm_counts = X.copy()
        else:
            import scanpy as sc
            import scipy.sparse as sp
            ...
            if isinstance(X, pd.DataFrame):
                ...
                X_mat = np.asarray(X)
                # If you pass ndarray, we won't know names; create dummy ones
                obs = pd.DataFrame(index=[f"cell_{i}" for i in range(X_mat.shape[0])])
                var = pd.DataFrame(index=[f"gene_{j}" for j in range(X_mat.shape[1])])

            if densify:
                norm_counts = sc.AnnData(X=X_mat.astype(np.float64), obs=obs, var=var)
            else:
                norm_counts = sc.AnnData(X=sp.csr_matrix(X_mat.astype(np.float64)), obs=obs, var=var)

        # --- Decide what to store as TPM (can be same as X if you don't have TPM) ---
        if tpm is None:
            tpm_adata = norm_counts.copy()
        else:
            if "AnnData" in str(type(tpm)):
                tpm_adata = tpm.copy()
            else:
                import scanpy as sc
                import scipy.sparse as sp
                ...
                if isinstance(tpm, pd.DataFrame):
                    obs = pd.DataFrame(index=tpm.index.astype(str))
                    var = pd.DataFrame(index=tpm.columns.astype(str))
                    tpm_mat = tpm.values
                else:
                    tpm_mat = np.asarray(tpm)
                    obs = pd.DataFrame(index=norm_counts.obs_names)
                    var = pd.DataFrame(index=norm_counts.var_names)

                if densify:
                    tpm_adata = sc.AnnData(X=tpm_mat.astype(np.float64), obs=obs, var=var)
                else:
                    tpm_adata = sc.AnnData(X=sp.csr_matrix(tpm_mat.astype(np.float64)), obs=obs, var=var)

        # --- Sanity: ensure matching obs/var between norm_counts and tpm ---
        if not (list(norm_counts.obs_names) == list(tpm_adata.obs_names)):
            raise ValueError("obs (cell/sample) names do not match between X and tpm.")
        if not (list(norm_counts.var_names) == list(tpm_adata.var_names)):
            raise ValueError("var (gene) names do not match between X and tpm.")

        # --- Write required artifacts ---
        import scanpy as sc
        sc.write(self.paths["normalized_counts"], norm_counts)
        sc.write(self.paths["tpm"], tpm_adata)

        # tpm_stats in the same format as prepare()
        import numpy as np
        import pandas as pd
        import scipy.sparse as sp

        if sp.issparse(tpm_adata.X):
            gene_mean, gene_var = get_mean_var(tpm_adata.X)
            gene_std = gene_var ** 0.5
        else:
            gene_mean = np.array(tpm_adata.X.mean(axis=0)).reshape(-1)
            gene_std  = np.array(tpm_adata.X.std(axis=0, ddof=0)).reshape(-1)

        tpm_stats = pd.DataFrame(
            [gene_mean, gene_std],
            index=["__mean", "__std"],
            columns=tpm_adata.var_names
        ).T
        save_df_to_npz(tpm_stats, self.paths["tpm_stats"])

        # nmf_genes_list (downstream expects this file)
        with open(self.paths["nmf_genes_list"], "w") as f:
            f.write("\n".join(list(norm_counts.var_names)))

        # NMF run/replicate parameter files (downstream expects these)

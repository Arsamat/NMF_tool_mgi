...
def get_groups_for_experiment(experiment: str) -> list:
    """
    Return one summary dict per unique group within an experiment.
    Fields: group, model_design, context, n_samples, sample_names.
    Multiple rows per group exist (one per term_label); we deduplicate.
    """
...
def get_terms_for_group(experiment: str, group: str) -> list:
    """
    Return available term_labels for a given experiment + group.
    ...
    docs = list(results_col.find(
        {"experiment": experiment, "group": group},
        {
            ...
            "gsea_barplot_path": 1, "gsea_barplot_exists": 1,
        },
    ))

...
def _ensure_research_session(state_prefix: str):
...
def render_deg_results_and_visualizations(
    ...
    """
    _ensure_research_session(state_prefix)
    ...
    with st.expander("Heatmap", expanded=True):
        ...
        hm_anno = st.session_state.get(f"{state_prefix}heatmap_annotation_df")
        if hm_df is not None and not hm_df.empty:
            ...
            if heatmap_html:
                st.download_button(
                    ...
                    "text/html",
                    key=f"{widget_prefix}heatmap_html_download",
                )
